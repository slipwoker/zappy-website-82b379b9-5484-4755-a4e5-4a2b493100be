document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getErrorContainer(field) {
      ...
      return field.parentElement.querySelector('.field-error, [role="alert"]') || null;
    }
    ...
    function showError(field, message) {
      ...
      let errEl = getErrorContainer(field);
      if (!errEl) errEl = createErrorElement(field);
    ...
    function clearError(field) {
      ...
      const errEl = getErrorContainer(field);
      if (errEl) {
        errEl.textContent = '';
    ...
    function validateField(field) {
      ...
      const label =
        (field.id && contactForm.querySelector('label[for="' + field.id + '"]'))
          ? contactForm.querySelector('label[for="' + field.id + '"]').textContent.replace('*', '').trim()
          : name;
    ...
    contactForm.addEventListener('blur', function (e) {
      ...
      if (!field) return;
      if (field.type === 'hidden' || field.type === 'submit' || field.disabled) return;
      validateField(field);
    }, true);
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      // Native form submission (action attribute) or custom event
      ...
      if (action && action !== '#') {
        ...
        fetch(action, {
          ...
          .catch(function (err) {
            ...
            const errorMsg = contactForm.querySelector('.form-general-error, [data-general-error]');
            if (errorMsg) {
        ...
        contactForm.dispatchEvent(new CustomEvent('formValidated', {
          bubbles: true,
          detail: { formData: new FormData(contactForm) }
        }));
    ...
    const mutationObserver = new MutationObserver(function (mutations) {
      mutations.forEach(function (mutation) {
        mutation.addedNodes.forEach(function (node) {
          ...
          const isAnimatable =
            node.hasAttribute(ANIMATION_ATTR) ||
          ...
          if (isAnimatable && !node.classList.contains(ANIMATED_CLASS)) {
            initScrollAnimations();
          }
        });
      });
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "ורד לוי משרד עורכי דין משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '82b379b9-5484-4755-a4e5-4a2b493100be',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '82b379b9-5484-4755-a4e5-4a2b493100be';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '82b379b9-5484-4755-a4e5-4a2b493100be';
